...
- each directed edge = flow of money from one transaction to another
...
Money laundering is not just about one bad transaction in isolation.  
It is usually a **network phenomenon**:
...
- routed through sequences of transactions that individually look harmless

So instead of only looking at raw tabular transaction attributes, this project models laundering as a **graph problem** and tries to capture:

1. **local structure**  
   Is a transaction mostly receiving? mostly sending? acting as a pass-through relay?

2. **meso-scale structure**  
   Does it sit inside a suspicious community / ring / laundering cluster?
...
From the transaction graph, I construct a set of **hand-designed graph features**.  
These are meant to capture common laundering behaviors such as:
...
- **bridging between two weakly connected regions**
...
### Structural features
- **scc_size** – size of strongly connected component  
...
This last one is one of the most interesting custom features in the project, so I explain it separately below.
...
- \(b_i\) close to 1 = transaction \(i\) looks illicit
- \(b_i\) close to 0 = transaction \(i\) looks licit

This base score is important because the next stage does **community-level refinement** on top of it.

So the base model answers:

> “If I only look at the transaction and its graph-derived features, how suspicious does it look?”
...
So after obtaining the base risk score \(b_i\) for every transaction, I run **community detection** on the graph to find transaction clusters.  
Inside each detected community, I then refine risk by letting suspiciousness interact with **who sends to whom inside the community**.

This creates a second-stage score that is not purely local anymore.
...
The point of the graph features is not “throw every centrality measure at the graph”.  
The goal is to encode behaviors that are actually meaningful for laundering.
...
So it behaves like a **relay / transit node** rather than a pure sink or pure source.

That matters because laundering chains often use intermediate nodes whose purpose is not to keep money, but to **receive and quickly forward it**.
...
High split score means one incoming stream is being dispersed into many outgoing branches.  
That is a classic laundering behavior because **splitting** increases traceability difficulty.
...
If a laundering operation uses a network of repeatedly interacting accounts / transactions, those nodes often lie deeper in the graph core rather than on the periphery.
...
Simple “burst count” features only count how many neighbors happen near the same time.  
That’s useful, but it misses **how concentrated the timing pattern is**.
...
Now form the empirical distribution over these offsets.  
If almost all neighbors happen in the same tiny time window, the distribution is highly concentrated.  
If neighbors are spread across many different offsets, the distribution is diffuse.

Let \(p_\delta\) be the fraction of neighbors with offset \(\delta\).  
Then the entropy is:
...
One laundering objective is to make it **hard to tell where the money ultimately goes**.  
That often means deliberately creating **many plausible downstream routes**.

So for each transaction node \(i\), I want to measure:

> If money starts at \(i\), how much uncertainty is there in its downstream flow paths?
...
At node \(i\), the one-step uncertainty is just the entropy of the outgoing transition distribution:
...
But laundering is not only about one hop.  
A node may have only 2 outgoing edges now, but those edges may open into a huge uncertain downstream tree.
...
- attribution of final destination becomes harder

So this feature is trying to quantify the laundering intuition:

> “This node is a good place to hide the true flow of funds because once money reaches here, the downstream path becomes uncertain.”
...
This score is purely transaction-level.  
It does **not yet know about community-level collective laundering behavior**.
...
The implementation uses a **Louvain-style modularity optimization** on the undirected version of the transaction graph.  
The idea is to partition the graph into communities so that nodes are more densely connected **within** communities than **across** them.
...
Then min-max normalize inside the community so that \(c_i\) is comparable as a community-local suspiciousness score.
...
So the final community-refined risk is not replacing the base model.  
It is saying:
...
# Important custom features in this project
