...
- a custom uncertainty / information-theoretic feature that measures how diffusely money can flow outward from a transaction
...
Money laundering is not just about one bad transaction in isolation. It is usually a **network phenomenon**:
...
From the transaction graph, I construct a set of **hand-designed graph features**. These are meant to capture common laundering behaviors such as:
...
Using the graph features from Stage 1, I train a **base MLP classifier** that outputs a transaction-level probability `b_i` in `[0, 1]`, where:

- `b_i` close to 1 means transaction `i` looks illicit
- `b_i` close to 0 means transaction `i` looks licit
...
```text
pass_through(i) =
min(indegree(i), outdegree(i)) / (max(indegree(i), outdegree(i)) + eps)
```
...
```text
b(i) = P(transaction i is illicit | graph features of node i)
```
...
Then min-max normalize inside the community so that `c(i)` becomes a community-local suspiciousness score.
