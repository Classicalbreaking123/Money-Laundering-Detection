...
def load_elliptic_raw(
    ...
    """
    Load raw Elliptic data using only:
    - labels from classes file
    - edges from edgelist file
    - txId + timestep from features file

    Ignores the built-in Elliptic feature columns for the main project.
    """
    ...
    # --------------------------------------------------
    # 2. Load txId + timestep from features file
    #    Supports either:
    #    - elliptic_txs_features.csv
    #    - elliptic_txs_features.zip
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    # 3. Load directed edges
    # --------------------------------------------------
    edges_df = pd.read_csv(edgelist_path)

    # Expected columns in elliptic_txs_edgelist.csv:
    # txId1, txId2
    edges = list(zip(edges_df["txId1"], edges_df["txId2"]))

    # --------------------------------------------------
    # 4. Keep only nodes that exist in both labels and timesteps
    # --------------------------------------------------
    ...
    # Also keep only nodes that actually appear in graph or metadata
    graph_nodes = set()
    ...
    # Filter labels / timesteps to only graph nodes
